...
def gen_NPV_df(m, df_cost, df_binary, n_month, cost_start_up):
    ...
    # DAC TOC
    TOC_DAC = value(m.x_cost_DAC_TPC) * (1 + 0.0311 + 0.0066 + 0.1779)
    ...
    # annual fuel cost
    fuel_cost_annual = sum(df_cost.loc[:, "cost_fuel"]) * w
    # annual CO2 transportation cost
    cost_CO2_TS = sum(df_cost.loc[:, "cost_CO2_TS"]) * w
    # annual FOM
    FOM_wo_DAC = 47965372
    FOM_total = FOM_wo_DAC + value(m.x_cost_DAC_FOM)
    # annual VOM
    VOM_total = df_cost[["cost_NGCC_VOM", "cost_PCC_VOM", "cost_DAC_VOM", "cost_PCC_compr_VOM", "cost_DAC_compr_VOM"]].sum().sum() * w
    # annual start-up cost
    start_up_cost_total = cost_start_up * max(sum(df_binary.loc[:, "z0"]) * w, 5)
    # cost excluding depreciation
    COST_EXCLUDE_D = np.zeros(22)
    for i in range(2, 21 + 1):
        COST_EXCLUDE_D[i] = fuel_cost_annual + cost_CO2_TS + FOM_total + VOM_total + start_up_cost_total

    annual_CO2_credit = sum(df_cost.loc[:, "credit_CO2"]) * w
    annual_power_sell = sum(df_cost.loc[:, "profit_power"]) * w
    # sell
    S = np.zeros(22)
