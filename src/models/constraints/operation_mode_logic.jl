...
function add_operation_mode_logic_constraints(model)
    ...
    z = model[:z]
    # 1. LAMBDA CONSTRAINT: DISPATCH ONLY, SUM OF EXTREME POINTS IS Y
    # when y = 0, z2 = 0, sum of lambda = 0, x_load_factor_D = 0
    # when y = 1, z2 = 1, start-up: x_load_factor_D[i, dispatch_idx] = 0
    # when y = 1, z2 = 0, normal dispatch mode: x_load_factor_D[i, dispatch_idx]
    ...
    # 3. MINIMUM ON-HOURS DURING START-UP
    @constraint(model, min_on_hours[i = set_hour_0],
        y[i] >= sum(z[i - k + 1] for k in set_start_up_hour if i - k + 1 >= 0)
    )
    ...
    @constraint(
        ...
        z2[i] 
        == 
        sum((z[i - k + 1]) for k in set_start_up_hour if i - k + 1 >= 0)
    )
